...
const Medicaments: React.FC = () => {
  ...
  const [filtered, setFiltered] = useState<Medicament[]>(medicamentsData);
  ...
  const clearSearch = () => {
    setSearchTerm('');
    // useRef : remet le focus dans le champ après avoir vidé la recherche
  ...
  const openEditModal = (item: Medicament) => {
    setEditingItem(item);
    setIsModalOpen(true);
  };

  const handleFormSubmit = (data: Omit<Medicament, 'id'>) => {
    if (editingItem) {
      setMedicaments((prev) =>
        prev.map((m) => (m.id === editingItem.id ? { ...data, id: editingItem.id } : m))
      );
    } else {
      const newItem: Medicament = { ...data, id: Date.now().toString() };
      setMedicaments((prev) => [...prev, newItem]);
    }
              ...
              {filtered.length === 0 ? (
                ...
                filtered.map((m, index) => {
                  ...
                  return (
                    <tr
                      key={m.id}
                      className="border-t border-gray-100 dark:border-gray-700 hover:bg-primary-50/40 dark:hover:bg-gray-700/40 transition-colors animate-fadeInUp"
